"""KVPO (KVPO) Trainer for autoregressive long-video generation.

Key design:
  * Rollout: use the frozen old policy to generate G perturbation branches,
    decode, score, and collect fork-block targets plus per-step denoising trajectories.
  * Update: per-step trajectory log-probabilities define branch weights; optimise a
    PPO/GRPO-style clipped ratio surrogate against the frozen old policy.
"""
...
class KVPOTrainer:
    ...
    def _normalize_kl_loss_mode(raw_mode: Any) -> str:
        ...
        if mode in {"discrete_kl", "discrete-kl", "distribution", "pi_kl", "branch_discrete_kl"}:
            return "discrete_kl"
        raise ValueError(
            "memflow KVPO only supports kl_loss_mode='discrete_kl' "
    ...
    def __init__(self, config, device, rank: int = 0, world_size: int = 1):
        ...
        self.long_reward_normal_clip_len = int(getattr(config, "long_reward_normal_clip_len", 48))
    ...
    def _normalize_policy_log_prob_mode(raw_mode) -> str:
        ...
        if mode not in aliases:
            raise ValueError(
                "memflow KVPO supports policy_log_prob_mode='per_step' "
    ...
    def rollout(self, prompts_list: List[str], sample_idx: int = 0) -> Dict[str, Any]:
        ...
        try:
            ...
            self.pipeline._initialize_kv_bank(batch_size, dtype, device)
            ...
            with torch.no_grad():
                for g in range(G):
                    ...
                    branch_rng.set_state(post_fork_rng)
        ...
        result = {
            ...
            "fork_abs_frame": fork_abs,
            "global_start_abs_frame": perturb_chunk_end_abs if need_global else None,
            ...
            "branch_trajectories": all_branch_trajectories,
        }
    ...
    def _build_postfork_clip_specs(
        self,
        postfork_pixel_len: int,
        prompts_list: List[str],
        switch_indices: List[int],
        global_start_abs: int,
    ) -> Tuple[List[ClipSpec], List[ClipSpec]]:
        """Build normal and transition ClipSpec lists for the post-global-start pixel region.

        Args:
            postfork_pixel_len: total pixel frames in the decoded post-global-start video.
            prompts_list: all segment prompts (full sequence, not post-global-start only).
            switch_indices: latent-space prompt switch frame indices (absolute).
            global_start_abs: latent frame where global scoring starts.

        Returns:
            (normal_specs, transition_specs)
        """
        fork_seg = self._get_segment_for_frame(global_start_abs, switch_indices)

        relative_switches_pixel: List[int] = []
        switch_seg_indices: List[int] = []
        for i, si in enumerate(switch_indices):
            if si > global_start_abs:
                rel_latent = si - global_start_abs
                rel_pixel = self._latent_to_pixel_frame(rel_latent)
                if 0 < rel_pixel < postfork_pixel_len:
                    relative_switches_pixel.append(rel_pixel)
                    switch_seg_indices.append(i)

        boundaries = [0] + relative_switches_pixel + [postfork_pixel_len]
        seg_prompts = prompts_list[fork_seg:]
        ...
        if self.long_reward_enable_normal:
            for i in range(len(boundaries) - 1):
                ...
                clip_start, clip_end = self._make_clip_window(
                    center, postfork_pixel_len, self.long_reward_normal_clip_len,
                )
                prompt = seg_prompts[min(i, len(seg_prompts) - 1)]
                normal_specs.append(ClipSpec(
                    name=f"normal_seg_{fork_seg + i}",
        ...
        if self.long_reward_enable_transition:
            for j, pixel_switch in enumerate(relative_switches_pixel):
                seg_idx = switch_seg_indices[j]
                clip_start, clip_end = self._make_clip_window(
                    pixel_switch, postfork_pixel_len, self.long_reward_transition_clip_len,
                )
                prompt_before = prompts_list[seg_idx]
                prompt_after = prompts_list[min(seg_idx + 1, len(prompts_list) - 1)]
                prompt = (
                    f"Transition between two consecutive scenes. "
                    f"First part: {prompt_before} "
                    f"Second part: {prompt_after} "
                    f"Judge whether the transition is visually stable and "
                    f"motion is natural across the boundary."
                )
                transition_specs.append(ClipSpec(
                    name=f"transition_{seg_idx}_{seg_idx + 1}",
                    start_frame=clip_start,
                    end_frame=clip_end,
                    prompt=prompt,
                    group="transition",
    ...
    def _compute_rewards_long(
        ...
        Returns:
            (reward_eval, videos) where reward_eval has per-component raw scores
            after weighted aggregation of normal and transition clip scores.
        """
        reward_components = reward_components or self.reward_components
        B = len(latent_list)
    ...
    def _denoise_blocks(
        ...
        def _generator_forward(noisy_in, cond, ts, kv, xattn, cur_start, bank, q_b):
    ...
    def _generate_postfork_anchor(
        ...
        kv_bank = self._materialize_cache_list(kv_saved["kv_bank1"])

        anchor_head = self._denoise_blocks(
            generator, noise_block, cond_at_fork,
            kv_cache, xattn_cache, kv_bank, fork_token,
        ...
        with torch.no_grad():
            ...
            generator(
                ...
                kv_bank=kv_bank, update_bank=False, update_cache=True, q_bank=True,
            )
        ...
        self.pipeline._restore_cache_list(self.pipeline.kv_bank1, kv_bank)
        del kv_cache, xattn_cache, kv_bank
    ...
    def _compute_trajectory_log_prob(
        ...
        def _gen_forward_single(noisy_in, cond, ts, kv, xattn, cur_start, bank, q_b):
            _, pred = generator(
                noisy_image_or_video=noisy_in,
                conditional_dict=cond,
                timestep=ts,
    ...
    def _discrete_kl_pi_vs_log_ref(
        ...
        """KL(π_θ || π_ref) for discrete π_θ, with log π_ref = log(π_ref + ε).

        Per-step training builds π from ``softmax`` over per-branch trajectory log-probs.
        """
    ...
    def _compute_kl_regularizer(
        ...
        """Discrete KL(π_θ || π_ref) for softmax π over branches."""
        return self._discrete_kl_pi_vs_log_ref(pi_theta, log_pi_ref), {}
    ...
    def train_step(self, rollout_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        local_window_is_safe_policy = any_branch_better
        global_has_safe_policy = self._distributed_bool_any(local_window_is_safe_policy)
        if self.is_main:
            ...
            print(
                ...
                f"local_window_is_safe={local_window_is_safe_policy} "
                f"global_has_safe={global_has_safe_policy} "
                f"(global: best={g_best} vs anchor={g_anc}, "
                f"local: best={l_best} vs anchor={l_anc})"
            )

        # --- Compute pi_old and run PPO epochs (per-step only) ---
        branch_trajectories = rollout_result.get("branch_trajectories")
        if not branch_trajectories or len(branch_trajectories) != G:
            raise ValueError(
                "memflow KVPO requires rollout_result['branch_trajectories'] with length G="
                f"{G} (got {len(branch_trajectories) if branch_trajectories else 0})."
            )
        ...
        for epoch in range(N):
            ...
            if self.per_step_protect_mode == "mechanism_1":
                if not local_window_is_safe_policy:
                    total_loss = torch.tensor(0.0, device=self.device, requires_grad=True)
                    if self.is_main and epoch == 0:
                        print(
                            "  [PROTECT] no branch beats anchor on any signal → zero policy loss "
                    ...
                    if self.kl_coef != 0.0:
                        log_pi_kl_ref = (
                            log_pi_initial
                            if (self.kl_reference_initial and log_pi_initial is not None)
                            else log_pi_old
                        )
                        ...
                        loss_kl_term, kl_info = self._compute_kl_regularizer(
                            pi_theta_branch,
                            log_pi_branch_ref,
                        )
                ...
                ).float().mean() if local_window_is_safe_policy else torch.tensor(0.0)
            else:
                m2_scale, m2_entropy, m2_pi_anchor = self._per_step_mechanism2_anchor_scale_entropy(
                    self._per_step_branch_log_pi(log_pi_theta), local_window_is_safe_policy
                )
                ratio_clipped = torch.clamp(ratio, self.ratio_clip_low, self.ratio_clip_high)
                surrogate = torch.minimum(ratio * fused_adv, ratio_clipped * fused_adv)
                loss_grpo_tensor = -surrogate.mean()
            ...
            info_policy = {
                ...
                "is_safe_local_window": local_window_is_safe_policy,
            ...
            if self.is_main:
                ...
                print(f"  epoch {epoch+1}/{N} [PER_STEP]: "
                      ...
                      f"safe_local_window={local_window_is_safe_policy} safe_global={global_has_safe_policy}{m2_dbg}")
        ...
        try:
            for ei in sorted(int(idx) for idx in prompt_indices):
                ...
                prompt_result: Dict[str, Any] = {
                    "prompt_idx": ei,
                    "fork_abs_frame": fork_abs,
                    "global_start_abs_frame": local_chunk_end,
    ...
    def _summarize_eval_results(
        ...
        mean_local_total = sum(local_totals) / len(local_totals) if local_totals else None
        mean_global_total = sum(global_totals) / len(global_totals) if global_totals else None
    ...
    def evaluate_both(
        ...
        """Evaluate online and EMA policies in one call."""
        results = [self.evaluate(eval_items, eval_seed=eval_seed, use_ema=False)]
        if self.ema is not None:
            results.append(self.evaluate(eval_items, eval_seed=eval_seed, use_ema=True))
