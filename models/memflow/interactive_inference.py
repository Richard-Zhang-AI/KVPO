...
from models.memflow.utils.distributed import barrier  
from models.memflow.utils.memory import gpu, get_cuda_free_memory_gb, DynamicSwapInstaller
from models.memflow.utils.prompt_sequences import resolve_switch_frame_indices

from pipeline.interactive_causal_inference import (
    InteractiveCausalInferencePipeline,
)
from models.memflow.utils.dataset import MultiTextDataset
...
def main():
    ...
    if os.environ.get("KVPO_FORCE_LOW_MEMORY", "").strip().lower() in {"1", "true", "yes", "on"}:
        low_memory = True
        print("KVPO_FORCE_LOW_MEMORY is enabled; forcing low-memory inference mode.")
    ...
    switch_frame_indices = resolve_switch_frame_indices(config.switch_frame_indices, num_segments)
    assert len(switch_frame_indices) == num_segments - 1, (
        "The number of effective switch_frame_indices should be the number of prompt segments minus 1"
    )
