...
from models.memflow.wan.modules.attention import attention
from models.memflow.wan.modules.model import (
    WanRMSNorm,
    rope_apply,
    WanLayerNorm,
...
from models.memflow.utils.memory import gpu, get_cuda_free_memory_gb, DynamicSwapInstaller, log_gpu_memory

from models.memflow.utils.debug_option import DEBUG
...
class CausalWanSelfAttention(nn.Module):
    ...
    def _build_perturbed_local_window(self, temp_k, temp_v, local_end_index, frame_seqlen, perturb_cfg):
        """Build a perturbed local KV window: m nearest frames + n randomly
        selected older frames.

        ``selected_older_frame_indices`` are **absolute** 0-based frame indices
        into the cache (frame 0 is the very first frame, i.e. sink position 0).
        The returned tensor replaces both the sink and the standard sliding
        window so that the caller should concatenate it with bank KV only.
        """
        m = perturb_cfg['m_nearest_frames']
        selected_older = perturb_cfg['selected_older_frame_indices']

        m_tokens = m * frame_seqlen
        nearest_start = max(0, local_end_index - m_tokens)
        ...
        for frame_idx in selected_older:
            ...
            if end <= nearest_start and end <= temp_k.shape[1]:
                k_parts.append(temp_k[:, start:end])
                v_parts.append(temp_v[:, start:end])

        if len(k_parts) > 0:
            k_older = torch.cat(k_parts, dim=1)
            v_older = torch.cat(v_parts, dim=1)
            k_local = torch.cat([k_older, k_nearest], dim=1)
            v_local = torch.cat([v_older, v_nearest], dim=1)
        else:
            k_local = k_nearest
            v_local = v_nearest
                ...
                if local_budget > 0:
                    ...
                    if (perturb_cfg is not None
                        ...
                        k_perturbed, v_perturbed = self._build_perturbed_local_window(
                            temp_k, temp_v, local_end_index, frame_seqlen, perturb_cfg)
                        if not is_recache:
                        ...
                        k_cat = torch.cat([k_bank, k_perturbed], dim=1)
                        v_cat = torch.cat([v_bank, v_perturbed], dim=1)
                    else:
