...
from models.longlive.wan.modules.attention import attention
from models.longlive.wan.modules.model import (
    WanRMSNorm,
    rope_apply,
    WanLayerNorm,
...
from models.longlive.utils.memory import gpu, get_cuda_free_memory_gb, DynamicSwapInstaller, log_gpu_memory

from models.longlive.utils.debug_option import DEBUG
...
class CausalWanSelfAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.eps = eps
        self._kv_perturb_config = None
        self._kv_perturb_cached_k = None
        self._kv_perturb_cached_v = None
    ...
    def _build_perturbed_local_window(
        self,
        temp_k,
        temp_v,
        local_end_index,
        ...
        """Build a perturbed local window from sink-preserved resident KV history.

        ``selected_older_frame_indices`` are absolute video-frame indices.
        For each perturb block, we remap them into the currently visible
        non-sink cache slots so the same historical frames are reused across
        the full perturbation window.

        Unlike the rolling-window remap path, we cache the selected older KV
        tensors on first use within a perturbation window and reuse the same
        memory tokens for subsequent perturb blocks. This mirrors memflow's
        "fixed memory token" behavior more closely.
        """
        m = perturb_cfg['m_nearest_frames']
        selected_older = perturb_cfg['selected_older_frame_indices']

        window_start = max(sink_tokens, local_end_index - local_budget_tokens)

        m_tokens = m * frame_seqlen
        nearest_start = max(window_start, local_end_index - m_tokens)
        k_nearest = temp_k[:, nearest_start:local_end_index]
        v_nearest = temp_v[:, nearest_start:local_end_index]

        if len(selected_older) == 0:
            return k_nearest, v_nearest

        if self._kv_perturb_cached_k is None or self._kv_perturb_cached_v is None:
            sink_frames = perturb_cfg.get('sink_size', sink_tokens // frame_seqlen)
            current_start_frame = current_start // frame_seqlen
            visible_non_sink_frames = max((local_end_index - sink_tokens) // frame_seqlen, 0)
            visible_non_sink_start = max(sink_frames, current_start_frame - visible_non_sink_frames)
            visible_older_end = current_start_frame - m

            k_parts = []
            v_parts = []
            for frame_idx in selected_older:
                if not (visible_non_sink_start <= frame_idx < visible_older_end):
                    continue
                cache_slot_frame = sink_frames + (frame_idx - visible_non_sink_start)
                start = cache_slot_frame * frame_seqlen
                end = start + frame_seqlen
                if start < sink_tokens or end > nearest_start or end > temp_k.shape[1]:
                    continue
                k_parts.append(temp_k[:, start:end].clone())
                v_parts.append(temp_v[:, start:end].clone())

            if len(k_parts) > 0:
                self._kv_perturb_cached_k = torch.cat(k_parts, dim=1)
                self._kv_perturb_cached_v = torch.cat(v_parts, dim=1)

        if self._kv_perturb_cached_k is not None and self._kv_perturb_cached_v is not None:
            k_local = torch.cat([self._kv_perturb_cached_k, k_nearest], dim=1)
            v_local = torch.cat([self._kv_perturb_cached_v, v_nearest], dim=1)
        else:
            k_local = k_nearest
            v_local = v_nearest
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            perturb_cfg = getattr(self, '_kv_perturb_config', None)
            use_perturb = (
                perturb_cfg is not None
                and perturb_cfg.get('target_start_token', current_start) <= current_start
                < perturb_cfg.get('target_end_token', current_start + 1)
            )
            ...
            if sink_tokens > 0:
                ...
                if local_budget > 0:
                    if use_perturb:
                        k_perturbed, v_perturbed = self._build_perturbed_local_window(
                            temp_k, temp_v, local_end_index, frame_seqlen,
                            perturb_cfg, local_budget, sink_tokens, current_start,
                        )
                        k_cat = torch.cat([k_sink, k_perturbed], dim=1)
                        v_cat = torch.cat([v_sink, v_perturbed], dim=1)
                    else:
                ...
                if use_perturb:
                    k_cat, v_cat = self._build_perturbed_local_window(
                        temp_k, temp_v, local_end_index, frame_seqlen,
                        perturb_cfg, self.max_attention_size, 0, current_start,
                    )
                else:
                    window_start = max(0, local_end_index - self.max_attention_size)
                    k_cat = temp_k[:, window_start:local_end_index]
                    v_cat = temp_v[:, window_start:local_end_index]

            x = attention(roped_query, k_cat, v_cat, deterministic=True)
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def set_kv_perturbation(self, perturb_config):
        """Propagate KV perturbation config to all self-attention blocks."""
        for block in self.blocks:
            block.self_attn._kv_perturb_config = perturb_config

    def clear_kv_perturbation(self):
        """Clear KV perturbation config from all self-attention blocks."""
        for block in self.blocks:
            block.self_attn._kv_perturb_config = None
            block.self_attn._kv_perturb_cached_k = None
            block.self_attn._kv_perturb_cached_v = None
