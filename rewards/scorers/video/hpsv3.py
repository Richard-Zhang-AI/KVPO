...
from ... import HPSV3_BASE_MODEL_PATH, HPSV3_CKPT_PATH
    ...
    def __init__(self, config_path=None, checkpoint_path=None, device='cuda', differentiable=False):
        ...
        if not os.path.exists(checkpoint_path):
            raise FileNotFoundError(
                "HPSv3 checkpoint not found. Expected a local file under "
                f"'{checkpoint_path}'. Put the shared weight file in checkpoints/HPSv3/."
            )
        ...
        if not os.path.exists(HPSV3_BASE_MODEL_PATH):
            raise FileNotFoundError(
                "HPSv3 base model not found. Expected the shared Qwen2-VL directory under "
                f"'{HPSV3_BASE_MODEL_PATH}'. Set KVPO_HPSV3_BASE_MODEL if you store it elsewhere."
            )
        model_config.model_name_or_path = HPSV3_BASE_MODEL_PATH
