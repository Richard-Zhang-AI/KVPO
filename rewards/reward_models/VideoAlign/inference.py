...
class VideoVLMRewardInference():
    ...
    @staticmethod
    def _expand_per_video(value, count, default=None):
        if value is None:
    ...
    def prepare_batch_from_frames(
        ...
        """
        Prepare a batch directly from frame tensors, skipping video-file loading.
        Args:
        ...
        default_fps = self.data_config.fps
        default_num_frames = self.data_config.num_frames
        default_max_pixels = self.data_config.max_frame_pixels
        default_sample_type = self.data_config.sample_type
        count = len(video_tensors)

        fps_list = self._expand_per_video(fps, count, default=default_fps)
        num_frames_list = self._expand_per_video(num_frames, count, default=default_num_frames)
        max_pixels_list = self._expand_per_video(max_pixels, count, default=default_max_pixels)
        source_fps_list = self._expand_per_video(source_fps, count, default=16.0)
        sample_type_list = self._expand_per_video(sample_type, count, default=default_sample_type)

        processed_videos = []
        for idx, tensor in enumerate(video_tensors):
            processed = process_wanvideo_tensor(
                tensor,
                fps=fps_list[idx],
                num_frames=num_frames_list[idx],
                src_fps=float(source_fps_list[idx]),
                sample_type=str(sample_type_list[idx]),
                max_pixels=max_pixels_list[idx],
            )
